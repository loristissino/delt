...
foreach($model->getMainPositions() as $mp)
{
  $this->menu[]=array('label'=>Yii::t('delt', 'Closing entry for «{item}»', array('{item}'=>$mp->currentname)), 'url'=>array('bookkeeping/closingjournalentry', 'slug'=>$model->slug, 'position'=>$mp->position));
}

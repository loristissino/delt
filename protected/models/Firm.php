...
class Firm extends CActiveRecord
{
  ...
  /**
   * Returns a data provider for the sections of the firm.
   * @param integer $pagesize the pagesize desired
   * @return CActiveDataProvider the sections of the firm
   */
  public function getSectionsAsDataProvider($pagesize=5000)
  {
    $sort = new CSort;
    $sort->defaultOrder = 'rank ASC';
    
    return new CActiveDataProvider(Section::model()->belongingTo($this->id), array(
      'pagination'=>array(
          'pageSize'=>$pagesize,
          ),
      'sort'=>$sort,
  ...
  {
    return Section::model()->belongingTo($this->id)->findAll();
  }
  ...
  */
  ...
  {
    $text = Yii::t('delt', '<a rel="license" href="http://creativecommons.org/licenses/by-sa/3.0/deed.{locale}" target="_blank"><img alt="Creative Commons License" style="border-width:0" src="https://licensebuttons.net/l/by-sa/3.0/88x31.png" width="88" height="31" /></a><br />
...
//  $md5 = md5(CJSON::encode($data['base'] . $data['accounts'] . $data['templates'] . $data['journalentries'] . $data['meta'] . $key));
  ...
  public function cacheStatementsData($level=1)
  {
    ...
    { 
      $entry = $this->_prepareEntriesWith($this->getAccountBalances($statement->position), $statement->getClosingDescription(), $rank--, 1, $date, $section);
    ...
    {
      $entry = $this->_prepareEntriesWith($template->getAccountsInvolved($this), $template->description, $rank--, 0, $date, $section);
  ...
  private function _prepareEntriesWith($accounts=array(), $description='...', $rank, $is_closing, $date, $section)
  {
    $totaldebit=0;
    $totalcredit=0;
    ...
    {
      $je = new Journalentry();
      $je->setDefaultsForAutomaticEntry($this, $description, $rank, $is_closing, $date, $section->id);
      $je->save(false);
